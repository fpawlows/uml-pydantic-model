...
from pydantic_model import UmlPrimitiveType, UmlClass, UmlInterface, UmlDataType, UmlEnumeration, UmlVisibilityEnum
...
def pydantic_field_serializer_func(field: Union['UmlElement', 'UmlIdReference']) -> dict:
...
def serialize_fields_to_id_reference(fields: List[str]) -> Callable:
    def decorator(original_class: Type[BaseModel]) -> Type[BaseModel]:
        for field in fields:
            field_serializer_func = field_serializer(field)(pydantic_field_serializer_func)
...
attribute = UmlAttribute(id="attribute_id", type=UmlPrimitiveType(id="primitive_type_id", name="primitive_type_name"))
parameter = UmlParameter(id="parameter_id", type=UmlPrimitiveType(id="primitive_type_id", name="primitive_type_name"))
