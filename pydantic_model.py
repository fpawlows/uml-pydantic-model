...
class UmlOperation(UmlNamedElement):
    ...
    return_type: Optional[Union['UmlPrimitiveType', 'UmlClass', 'UmlInterface', 'UmlDataType', 'UmlEnumeration', 'UmlIdReference']] = None
    visibility: UmlVisibilityEnum = UmlVisibilityEnum.PUBLIC
    is_static: Optional[bool] = None
    is_ordered: Optional[bool] = None
    is_unique: Optional[bool] = None
...
class UmlEnclosedLifelinePart(UmlOccurrenceSpecification):
    ...
    def validate_occurences(cls, occurences: List['UmlOccurrenceSpecification']):
        ...
        if not all(occurence.lifeline == occurences[0].lifeline for occurence in occurences):
            raise ValueError("All occurences must be on the same lifeline.")
...
class UmlCoveringFragment(UmlElement):
    covered: List[Union['UmlEnclosedLifelinePart', 'UmlIdReference']] = Field(default_factory=list)

    @field_serializer("covered")
    def covered_to_json(covered: List[Union['UmlEnclosedLifelinePart', 'UmlIdReference']]) -> List[dict]:
...
class UmlLifeline(UmlNamedElement):
    covered_by: List[Union['UmlEnclosedLifelinePart', 'UmlOccurrenceSpecification']] = Field(default_factory=list)
    represents: Union['UmlClass', 'UmlInterface', 'UmlIdReference']

    @field_serializer("represents")
    def represents_to_json(represents: Union['UmlClass', 'UmlInterface', 'UmlIdReference']) -> dict:
...
class UmlMessage(UmlNamedElement):
    ...
    @field_serializer("source")
    def source_to_json(source: Union['UmlOccurrenceSpecification', 'UmlIdReference']) -> dict:
        return serialize_field_to_id_reference(source)
    
    @field_serializer("target")
    def target_to_json(target: Union['UmlOccurrenceSpecification', 'UmlIdReference']) -> dict:
        return serialize_field_to_id_reference(target)
    
    @field_serializer("signature")
    def signature_to_json(signature: Optional[Union['UmlOperation', 'UmlIdReference']]) -> dict:
...
class1 = UmlClass(
    ...
    name="Person",
    attributes=[
        UmlAttribute(name="name", type=UmlPrimitiveType(id="type1", name="String", type=UmlPrimitiveTypeTypesEnum.STRING)),
        UmlAttribute(name="age", type=UmlPrimitiveType(id="type2", name="Integer", type=UmlPrimitiveTypeTypesEnum.INTEGER))
    ],
    operations=[
        UmlOperation(name="getName", return_type=UmlPrimitiveType(id="type1", name="String", type=UmlPrimitiveTypeTypesEnum.STRING))
    ]
)
...
class2 = UmlClass(
    ...
    name="Address",
    attributes=[
        UmlAttribute(name="street", type=UmlPrimitiveType(id="type1", name="String", type=UmlPrimitiveTypeTypesEnum.STRING)),
        UmlAttribute(name="zipcode", type=UmlPrimitiveType(id="type2", name="Integer", type=UmlPrimitiveTypeTypesEnum.INTEGER))
    ]
)
...
interface1 = UmlInterface(
    ...
    operations=[
        UmlOperation(name="getAge", return_type=UmlPrimitiveType(id="type2", name="Integer", type=UmlPrimitiveTypeTypesEnum.INTEGER))
    ]
)
...
message1 = UmlMessage(
    id="message1",
    name="RequestAddress",
    source=UmlOccurrenceSpecification(id="occ1", lifeline=UmlIdReference(idref="lifeline1")),
    target=UmlOccurrenceSpecification(id="occ2", lifeline=UmlIdReference(idref="lifeline2")),
